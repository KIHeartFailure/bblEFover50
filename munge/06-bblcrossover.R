...
matchp <- matchplm2 %>%
  mutate(bblcensdate = case_when(shf_bbl == "No" ~ EDATUM,
                              shf_bbl == "Yes" & EDATUM <= ymd("2017-07-01") ~ EDATUM + 31 * 3,
         ...
         sos_out_hosphf_bbl = case_when(sos_outtime_hosphf_bbl < sos_outtime_hosphf ~ "No", 
                                        TRUE ~ as.character(sos_out_hosphf)),
         sos_out_deathcv_bbl = case_when(sos_outtime_death_bbl < sos_outtime_death ~ "No", 
