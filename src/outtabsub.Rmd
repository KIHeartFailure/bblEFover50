...
survmysub <- function(time, event, xvar, xvarname) {
  ...
  for (i in seq_along(levs)) {
    ...
    mod <- coxph(formula(paste0(
      "Surv(", time, ",",
      event, "=='Yes') ~ shf_bbl * relevel(", xvar, ", ref = '", levs[i], "') + frailty(par)"
    )),
    data = matchp
    )
    ...
    out[1 + i, 4] <- log(smod$conf.int[1, 1])
    out[1 + i, 5] <- log(smod$conf.int[1, 3])
    out[1 + i, 6] <- log(smod$conf.int[1, 4])

    if (i == 1) {
      pint <- car::Anova(mod, type = "III")

      out[1, 3] <- dF(pint[4, 3], dig = 3, p = TRUE)
    }
  }
  return(out)
}
...
plotforest <- function(forestdata) {
  ...
  axis(1,
    cex.axis = cextext, at = log(seq(0.5, 1.3, 0.1)),
    labels = c(0.5, 0.6, 0.7, 0.8, NA, 1.0, NA, NA, 1.3)
  )
  axis(1,
    cex.axis = cextext, at = log(c(0.9, 1.1)),
    labels = c(0.9, 1.1)
  )
